...
/*     function mintToken(uint256 price, uint256 _noOfTracks, string memory _tracklink, string memory newuri) external {
    ...
    function mintToken(uint256 price, uint256 _noOfTracks, string memory _tracklink, string memory newuri) external onlyOwner {
        address payable _artist = payable(msg.sender);  
        NFT memory newNFT = NFT(nftId,_artist,price,_noOfTracks, 0);
        track_link[nftId] = _tracklink;
        nfts[nftId] = newNFT;
        _mint(msg.sender, nftId, _noOfTracks, "");
        _addRecord(_nftID, _noOfTracks);
        nftId++;
    }

    function _addRecord(uint256 _nftID, uint256 _noOfTracks) private {
        uint mintIndex = totalSupply(_nftID).add(_noOfTracks);
        _mint(ADMIN_ADDRESS, _nftID, _noOfTracks, "0x0");
    }
